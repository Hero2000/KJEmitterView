...
//    UIImage *img = [self.smallImageView.image kj_imageTiledWithTiledType:(tiledType) TargetImageSize:KJImageSizeMake(self.view.bounds.size.width, self.view.bounds.size.width) Row:self.rowSte.value Col:self.colSte.value];
    UIImage *img = [KJWallpaperPave kj_wallpaperPaveWithMaterialImage:self.smallImageView.image TiledType:tiledType TargetImageSize:CGSizeMake(self.view.bounds.size.width, self.view.bounds.size.width) Width:87];
    self.imageView.image = img;
}
- (IBAction)row:(UIStepper*)sender {
    UIImage *img = [KJWallpaperPave kj_wallpaperPaveWithMaterialImage:self.smallImageView.image TiledType:tiledType TargetImageSize:CGSizeMake(self.view.bounds.size.width, self.view.bounds.size.width) Row:self.rowSte.value Col:self.colSte.value];
    self.imageView.image = img;
    self.rowLabel.text = [NSString stringWithFormat:@"行：%d", (int)self.rowSte.value];
}
...
- (IBAction)button:(UIButton *)sender {
    self.smallImageView.image = [_KJIFinishTools kj_rotationImageWithImage:self.smallImageView.image Orientation:UIImageOrientationLeft];//[self.smallImageView.image kj_rotationImageWithOrientation:(UIImageOrientationLeft)];
    UIImage *img = [KJWallpaperPave kj_wallpaperPaveWithMaterialImage:self.smallImageView.image TiledType:tiledType TargetImageSize:CGSizeMake(self.view.bounds.size.width, self.view.bounds.size.width) Row:self.rowSte.value Col:self.colSte.value];
    self.imageView.image = img;
}
